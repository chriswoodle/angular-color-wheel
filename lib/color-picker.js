        ...
        return {
            ...
            link: function (scope, elem, attrs) {
                ...
                function init() {
                    ...
                    absoluteCenter = {
                        x: elem[0].offsetLeft + relativeCenter.x,
                        y: elem[0].offsetTop + relativeCenter.y
                    }
                    r = paletteElem.clientWidth / 2 * .8; // 80% of half the width of the pallet
                    moveHandle({
                        x: absoluteCenter.x + r * Math.cos(angle),
                        y: absoluteCenter.y + r * Math.sin(angle)
                    });
                }
                elem.bind('mousedown', function (e) {
                    ...
                    moveHandle(e);
                    elem[0].addEventListener('mousemove', moveHandle, false);
                });
                elem.on('mouseup', function () {
                    elem[0].removeEventListener('mousemove', moveHandle);
                });
                ...
                function moveHandle(e) {
                    angle = getAngle(absoluteCenter, {
                        x: e.x,
                        y: e.y
                    });
                    onAngleChange();
                    handleElem.style.left = relativeCenter.x + r * Math.cos(angle) - handleElem.clientWidth / 2;
                    handleElem.style.top = relativeCenter.y + r * Math.sin(angle) - handleElem.clientHeight / 2;
                }
